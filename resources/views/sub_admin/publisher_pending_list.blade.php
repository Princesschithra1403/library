                        ...
                        @foreach($publisher as $val)
                          ...
                          <td> <span class="badge bg-success text-white">Approve</span></td>
                           ...
                           <td><span class="badge light badge-success">{{$val->created_at->format('Y-m-d')}}</span>
                              <td>
                                 <a href="/sub_admin/pub_profile/{{$val->id}}"><i class="fa fa-eye p-2"></i></a>
              ...
              $('#user_approval').on('change',function(){
                  ...
                  if(approval_ == 'Reject'){
                     ...
                     $.ajax({
                        type: "post",
                        dataType: "json",
                        url: '/sub_admin/approvestatus',
                        data: {'status': approval_ , 'publisherid': publisherid},
                        success: function(response) {
                           if(response.success){
                           setTimeout(function() {
                              window.location.href ="/sub_admin/publisher_pending_list"
                                 }, 3000);
